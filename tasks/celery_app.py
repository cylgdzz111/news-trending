...
celery_app.conf.beat_schedule = {
    # 微博热搜
    "crawl_weibo_hourly": {
        "task": "tasks.crawler_tasks.crawl_weibo",
        "schedule": crontab(minute=0),  # 每小时整点
        "args": ()
    },
    # 百度热搜
    "crawl_baidu_hourly": {
        "task": "tasks.crawler_tasks.crawl_baidu",
        "schedule": crontab(minute=10),  # 每小时10分
        "args": ()
    },
    # 知乎热榜
    "crawl_zhihu_hourly": {
        "task": "tasks.crawler_tasks.crawl_zhihu",
        "schedule": crontab(minute=20),  # 每小时20分
        "args": ()
    },
    # 抖音热点
    "crawl_douyin_hourly": {
        "task": "tasks.crawler_tasks.crawl_douyin",
        "schedule": crontab(minute=30),  # 每小时30分
        "args": ()
    },
    # B站热门
    "crawl_bilibili_hourly": {
        "task": "tasks.crawler_tasks.crawl_bilibili",
        "schedule": crontab(minute=40),  # 每小时40分
        "args": ()
    },
    # 清理过期数据
    "clean_expired_data_daily": {
        "task": "tasks.crawler_tasks.clean_expired_data",
